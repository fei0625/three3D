<template>
  <div class="popup-result">
    <p class="title">{{ getI18nValue('cron.runtimes') }}</p>
    <ul class="popup-result-scroll list-disc pl-6">
      <template v-if="isShow">
        <li v-for="item in resultList" :key="item" class="mt-2 first:mt-0">{{ item }}</li>
      </template>
      <li v-else>{{ getI18nValue('cron.calculation') }}</li>
    </ul>
  </div>
</template>
...
const dayRule = ref('')
const dayRuleSup = ref<any | string>('')
const dateArr = ref<any | number>([])
const resultList = ref<any>([])
const isShow = ref(false)
const props = defineProps<{ ex: any }>()
...
function expressionChange() {
  ...
  let MIdx: any = getIndex(MDate, nMouth)
    ...
    goMouth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
      ...
      goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
        ...
        if (dayRule.value == 'lastDay') {
          ...
          if ((Array as any).indexOf(dayRuleSup.value, thisWeek) < 0) {
            // 如果到达最大值时
            if (Di == DDate.length - 1) {
              resetDay()
              if (Mi == MDate.length - 1) {
  ...
  if (resultArr.length == 0) {
    resultList.value = [getI18nValue('cron.conditions')]
  } else {
    resultList.value = resultArr
    if (resultArr.length !== 5) {
      resultList.value.push(getI18nValue('cron.here') + resultArr.length + getI18nValue('cron.results'))
    }
  }
  // 计算完成-显示结果
  isShow.value = true
